...
from llm import DadosNotaFiscal, extrair_texto_pdf, extrair_dados_llm
...
if arquivo_upload is not None:
    if st.button("Processar Documento"):
        with st.spinner("Lendo e analisando o documento..."):
            ...
            texto_extraido = DadosNotaFiscal.model_validate(extrair_texto_pdf(arquivo_upload))
            
            if not texto_extraido.strip():
                st.error("O PDF parece estar vazio ou é uma imagem escaneada. OCR é necessário.")
            else:
                # Extrair dados estruturados usando LLM
                dados_estruturados = DadosNotaFiscal.model_validate(extrair_dados_llm(texto_extraido))
